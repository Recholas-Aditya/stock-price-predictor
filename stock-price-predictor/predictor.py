...
df["SMA_7"] = df[TARGET_COL].rolling(window=7).mean().shift(1)
df["SMA_21"] = df[TARGET_COL].rolling(window=21).mean().shift(1)
...
for i in range(PREDICT_N_DAYS):
    ...
    current_indicators = np.array([
        pd.Series(close_history).rolling(window=7).mean().iloc[-1],
        pd.Series(close_history).rolling(window=21).mean().iloc[-1],
